var NOTE_DOUBLE_FLAT  = [ 'daw', 'raw','maw','faw','saw', 'law','taw','daw','raw','maw','faw','saw', 'law','taw','daw', ];
var NOTE_FLAT  =        [ 'de' , 'ra', 'me', 'fe', 'se',  'le', 'te', 'de' ,'ra', 'me', 'fe', 'se',  'le', 'te', 'de',  ];
var NOTE_NATURAL =      [ 'do' , 're', 'mi', 'fa', 'sol', 'la', 'ti', 'do', 're', 'mi', 'fa', 'sol', 'la', 'ti', 'do',  ];
var NOTE_SHARP =        [ 'di' , 'ri', 'ma', 'fi', 'si',  'li', 'ta', 'di' ,'ri', 'ma', 'fi', 'si',  'li', 'ta', 'di',  ];
var NOTE_DOUBLE_SHARP = [ 'dai', 'rai','mai','fai','sai', 'lai','tai','dai','rai','mai','fai','sai', 'lai','tai','dai', ];
...
var NOTES_CROMATIC_VALUE = [
	[ 2 , 4, 6, 7, 9, 11, 13, 14, 16, 18, 19, 21, 23, 25, 26 ],
	[ 1 , 3, 5, 6, 8, 10, 12, 13, 15, 17, 18, 20, 22, 24, 25 ],
	[ 0 , 2, 4, 5, 7,  9, 11, 12, 14, 16, 17, 19, 21, 23, 24 ],
	[-1 , 1, 3, 4, 6,  8, 10, 11, 13, 15, 16, 18, 20, 22, 23 ],
	[-2 , 0, 2, 3, 5,  7,  9, 10, 12, 14, 15, 17, 19, 21, 22 ],
];
...
NoteLoc.prototype.getChromaticIndex = function () {
	return NOTES_CROMATIC_VALUE[this.ci][this.ni];
};
...
function __lookup( root, intervals ) {
	if ( Array.isArray( intervals ) ) {
		var result = [];
		for ( var i=0; i<intervals.length; i++ ) {
			result.push( lookup( root, intervals[i] ) );
		}
		return result;
	} else {
		return lookup( root, intervals );
	}
}
...
function __enharmonize( note, wrapper ) {
	if ( Array.isArray( note ) ) {
		note = Array.prototype.slice.call( note );
		for ( var i=0; i<note.length; i++ ) {
			note[i] = enharmonize( note[i], wrapper );
		}
		return note;
	} else {
		return enharmonize( note, wrapper );
	}
}
...
if ( module && module.exports ) {
	module.exports.lookup = __lookup;
...
if ( require && module && require.main === module )  {
	...
	argv.shift();
	argv.shift();
	process.stdout.print( __lookup( argv.shift(), argv ).join( ' ' ) );
}
