...
function transpose( root, intervals ) {
	// 1. if the first argument is a string value, split it to an array.
	if ( typeof root === 'string' ) {
		let tmp = root.trim().split( /\s+/ );
		if ( 1 < tmp.length ) {
			root = tmp;
		} else {
			// do nothing. treat it as string.
		}
	}

	// 2.  if the first argument is an array, treat it as entire arguments.
	if ( Array.isArray( root ) )
		[ root ,intervals ] = [ root.shift(), root ];

	// console.error( root,  intervals );
	// console.error( typeof root,  typeof intervals );
