...
var NOTE_QUADRUPLE_FLAT  = triple( [ 'daws','raws','maws','faws','saws','laws','taws', ] );
...
var NOTE_QUADRUPLE_SHARP = triple( [ 'dais','rais','mais','fais','sais','lais','tais', ] );
...
function __transpose( root, interval ) {
	...
	var parsedInterval = /([a-zA-Z]+)([,']*)/.exec( interval );
	if ( ! parsedInterval )
...
function transpose( /* root:array of string, intervals : array of string */ ) {
	function interpolateScales( root, intervals ) {
		let scale = null;
		{
			...
			for ( var i=0; i<intervals.length; i++ ) {
				var matched2 = /([^0-9]*)([0-9]+)([^0-9]*)/.exec( intervals[i] );
				if ( matched2 ) {
					if ( scale == null ) {
						throw new Error( 'scale not defined error' );
					}
					var n = (Number( matched2[2] ) -1) % scale.length;
					intervals[i] = matched2[1] + scale[n] + matched2[3];
				}
			}
	...
	function multi_executeProc( root, intervals ) {
		[ root, intervals ] = interpolateScales( root, intervals );

		var result = [];
		for ( var i=0; i<intervals.length; i++ ) {
			result.push( __transpose( root, intervals[i] ) );
		}
		return result;
	}
	...
	// argument count => 1
		...
		var [ root ,intervals ]  = [ value.shift(), value ] ;

		return multi_executeProc( root, intervals );
...
function commandInterface( argv ) {
	...
	if ( argv.length == 0 ) {
		...
		switch ( type ) {
			case 'ch' :
			case 'chromatic' :
				commandInterfaceProc( transpose, argv ); 
				break;

			case 'respell' :
				commandInterfaceProc( wrapSingleConvFunc( respell ), argv ); 
				break;
			case 'note2number' :
				commandInterfaceProc( wrapSingleConvFunc( note2number ), argv ); 
				break;
			case 'number2note' :
				commandInterfaceProc( wrapSingleConvFunc2( number2note ), argv ); 
				break;
			case 'note2alphabet' :
				commandInterfaceProc( wrapSingleConvFunc( note2alphabet ), argv ); 
				break;
			case 'note2alphabet_tex' :
				commandInterfaceProc( wrapSingleConvFunc( note2alphabet_tex ), argv ); 
				break;
			default : 
...
if ( module && module.exports ) {
	...
	module.exports.isQuadrupleSharp      = isQuadrupleSharp;
	module.exports.isQuadrupleFlat       = isQuadrupleFlat;
	module.exports.isQuadrupleAccidental = isQuadrupleAccidental;
	...
	module.exports.note2alphabet         = note2alphabet;
	module.exports.note2alphabet_tex     = note2alphabet_tex;
