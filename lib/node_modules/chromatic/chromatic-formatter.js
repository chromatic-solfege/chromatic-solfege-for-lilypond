...
function init( __settings ) {
	...
	function transpose( value, settings ) {
		return require( 'chromatic' ).transposeScript( value, settings ).join( " " )
	}
	...
	ScaleGenerator.prototype.writeScore = function () {
		...
		if ( curr_settings.formatType && curr_settings.formatType.startsWith( 'query' )  ) {
			let lilypondCommandLine = require( "chromatic/lilyutils" ).getLilypondCommandLine();
			let queryFileName = this.outputPath + "query-fretdiagram.ly";
			let resultFileName = queryFileName + '.LOG';
			...
			require('child_process').execSync( lilypondCommandLine +  ' "' + queryFileName + '" > "' + resultFileName + '"' );
	...
	ScaleGenerator.prototype.writeHeaderVoice = function( caption, prefix ) {
        prefix = prefix || "Header";
		var id = prefix + '-' + caption2id( caption );
		this.nextOutputFilename( id, 'ftxt' );
		var filename = this.currentOutputFilename();

		ScaleGenerator.writeFile( filename , caption );
	...
	ScaleGenerator.prototype.writeAbstract = function(caption) {
		...
		this.writeHeaderVoice( 
                this.settings.festivalFilter( 
                    /*type*/ 'abstract', 
                    /*caption*/"Abstract" ).festival, "Abstract" );
        // <<< (Tue, 11 Feb 2020 11:05:10 +0900)

		var filtered  = this.settings.festivalFilter( 'abstract', caption );

        // Let it say as text body.
        // >>> (Wed, 12 Feb 2020 02:55:59 +0900)
		// this.writeHeaderVoice( filtered.festival, "Abstract" );
		this.writeHeaderVoice( filtered.festival, "TextBody" );
        // <<< (Wed, 12 Feb 2020 02:55:59 +0900)
