...
function init( __settings ) {
	...
	ScaleGenerator.prototype.writeScore = function () {
		...
		// console.error( id );
		if ( Array.isArray( generatedSettings.lyTextAfter  ) ) {
			if ( generatedSettings.lyTextAfter.length == 0 ) {
				generatedSettings.lyTextAfter = [5,3];
			}
			{
				let [ __h, __w ] = generatedSettings.lyTextAfter;
				generatedSettings.lyTextAfter = 
					`
					\\paper  {
		...
		var output = cht.applyTemplate( notes,  this.valueCurrentOutputFilename, this.templatePath, generatedSettings );

		{
			let lyTextBefore = generatedSettings.eventhandler.call( 
					this, 'before', id, notes, generatedSettings.lyTextBefore );
			if ( lyTextBefore )
				output = lyTextBefore + '\n\n' + output;
		}

		{
			let lyTextAfter  = generatedSettings.eventhandler.call( 
					this, 'after',  id, notes, generatedSettings.lyTextAfter );
			if ( lyTextAfter )
				output = output + '\n\n' + lyTextAfter + '\n';
		}
		...
		this.writeIncludeText(
				generatedSettings.eventhandler.call( this, 'tex',id, notes, 
					'\\noindent $\\vcenter{ \\hbox{ \\includegraphics[width=' + 
					generatedSettings.texGraphicWidth + 
					' \\textwidth]{' + 
	...
	function commandInterface() {
		...
		var notes = args.join( ' ' );
		// console.error( notes );
		process.stdout.write( cht.applyTemplate( notes, null, null, this.settings ) );
