...
function init( argSetting ) {
	var setting = {
		voiceFilter :(type, s)=>s,
	};
	Object.assign( setting, argSetting );
	...
	ScaleGenerator.prototype.writeScore = function () {
		...
		console.error( "ScaleGenerator.settings", this.settings );
		var output = ScaleGenerator.applyTemplate( notes,  this.valueCurrentOutputFilename , this.templatePath, this.settings );
	...
	ScaleGenerator.prototype.writeHeaderVoice = function( type, caption ) {
		...
		filecontent = setting.voiceFilter( type, filecontent );

		fs.writeFileSync( filename + ".txt" , filecontent , 'utf8' );

		require('child_process').execSync( 'text2wave -eval "(' + ScaleGenerator.voiceName + ')" ' + filename + ".txt" + '>' + filename + ".wav" );
	}
