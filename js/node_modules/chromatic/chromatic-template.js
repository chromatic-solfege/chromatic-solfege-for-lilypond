		...
		switch ( settings.formatType  ) {
			...
			case 'diagram':
				result += ( r`
					...
					#(put-markup-on-a-note! 
						(cdr (assq 'accepted-notes scale-diagram-def))
						'first
			...
			case 'queryFretDiagram':
				result += ( r`
					...
					#(format #t "{ \"length\" : ~a, \"start-pos\" :~a, \"end-pos\" : ~a , \"notes\" : [ ~a ] }\n"
						 (cdr (assq 'length    overhang-def))
						 (cdr (assq 'start-pos overhang-def))
						 (cdr (assq 'end-pos   overhang-def))
						 (let* ((accepted-pitches
								 (cdr (assq 'accepted-pitches  scale-diagram-def)))
								(quoted-pitches (map
												 (lambda (v)
													 (string-append
													  "\""
								...
								(chopped-pitches (string-concatenate (reverse (cdr (reverse comma-added-pitches ))))))
							 chopped-pitches))
					`).replace( /^\s{5}/gm, '' );
				break;
