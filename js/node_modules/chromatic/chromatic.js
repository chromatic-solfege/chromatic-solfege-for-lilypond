function chromatic( __settings, __object ) {
	...
	NoteLoc.prototype.translate = function() {
		var ci = arguments[0];
		var ni = arguments[1];
		console.error( `translate ( ${ci}, ${ ni } ) on NoteLoc( ${ this.ci } , ${this.ni } ) ` );
		...
		for ( var ci = 0; ci< NOTES_ALL.length; ci++ ) {
			...
			for ( var ni = 0; ni< NOTES_ALL[ci] .length; ni++ ) {
				...
				if ( NOTES_ALL[ci][ni] == noteName ) {
					console.error( 'found ' + noteName +'[' + ci + ',' + ni + ']' );
	...
	function __transpose( root, interval ) {
		...
		return __transpose0( root, interval ) ;
	}
